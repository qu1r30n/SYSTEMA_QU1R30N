...
char **leer_archivo(const char *ruta, int *n_lineas_out) // carga el archivo completo en memoria para procesar sus filas // ejemplo: inventario completo
{
    ...
    char buffer[4096]; // declara una variable que se usará en las operaciones siguientes // ejemplo: buffer temporal

    while (fgets(buffer, sizeof(buffer), f)) // itera mientras la condición siga siendo verdadera // ejemplo: puntero != NULL
    {
        if (n >= capacidad) // evalúa la condición para decidir si entra al bloque actual // ejemplo: ruta != NULL
        {
            ...
            char **temp = (char **)realloc(lineas, capacidad * sizeof(char *)); // redimensiona memoria dinámica para ampliar o ajustar la capacidad actual // ejemplo: duplicar el arreglo de punteros
            ...
            lineas = temp; // actualiza la variable con el nuevo valor calculado en esta etapa // ejemplo: total = 0
        }

        buffer[strcspn(buffer, "\n")] = 0; // localiza la primera coincidencia del conjunto buscado para recortar el texto // ejemplo: quitar salto de línea

        lineas[n] = (char *)malloc(strlen(buffer) + 1); // reserva memoria dinámica para almacenar el dato o buffer requerido // ejemplo: espacio para una nueva cadena
        if (!lineas[n]) // evalúa la condición para decidir si entra al bloque actual // ejemplo: ruta != NULL
        {
            free_lineas(lineas, n); // libera el arreglo de líneas y sus cadenas asociadas // ejemplo: contenido del archivo leído
            fclose(f); // cierra el archivo abierto para liberar el descriptor del sistema // ejemplo: termina la lectura
            *n_lineas_out = 0; // escribe el valor indicado en la dirección apuntada por el puntero // ejemplo: *n_lineas_out = 0
        ...
        strcpy(lineas[n], buffer); // copia una cadena completa al destino indicado // ejemplo: duplicar la ruta original
        imprimirMensaje_para_depurar("\n%s", lineas[n]); // envía un mensaje de depuración para seguir el flujo de ejecución // ejemplo: ruta creada
        n++; // mantiene esta instrucción dentro del flujo actual sin alterar la lógica original // ejemplo: paso intermedio del proceso
    }
